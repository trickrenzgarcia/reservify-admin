    ...
    <div className='w-full flex flex-col'>
      ...
      <div className='px-4 lg:px-28'>
        <ScrollArea className='h-[calc(100vh-220px)]'>
          {data.map((review) => (
            <div key={review.id}>
              <Card className='bg-[#e1e1e1] pt-10 px-10 rounded-3xl'>
                <CardContent className='flex flex-col lg:flex-row'>
                  <Avatar className='w-[150px] h-[150px]'>
                    <AvatarImage src='' alt='' />
                    <AvatarFallback className='text-3xl font-bold'>{review.customer.name.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div className='flex flex-col px-4 py-4 w-full'>
                    <div className='flex flex-col lg:flex-row justify-between'>
                      <h1 className='text-3xl font-bold'>{review.customer.name}</h1>
                      <div className='flex'>
